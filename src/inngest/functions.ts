...
async({event,step})=>{
    ...
    const transcriptWithSpeakers=await step.run("add-speakers",async()=>{
       const speaker_ids=[...new Set(transcript.map((item)=>item.speaker_id))]
       const userSpeakers=await db.select().from(user).where(inArray(user.id,speaker_ids)).then((users)=>users.map((user)=>({...user,})))
       const agentSpeakers=await db.select().from(agents).where(inArray(agents.id,speaker_ids)).then((users)=>users.map((agents)=>({...agents,})))
